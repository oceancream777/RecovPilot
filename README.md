...
- [Live/Demo Telemetry Routing](#livedemo-telemetry-routing)
...
- `GET /api/v1/telemetry/status` reports whether verified live Razorpay telemetry is active or the dashboard has returned to demo mode.
- `GET /api/v1/health` reports API, database, and model availability.

## Live/Demo Telemetry Routing

The dashboard has two input sources, selected automatically by the backend's verified-event state.

```mermaid
flowchart LR
    A[React dashboard polls telemetry status] --> B{Verified payment.failed received recently?}
    B -->|No| C[Demo mode]
    C --> D[Scenario dropdown and editable synthetic inputs]
    B -->|Yes| E[Live mode]
...
The signed listener activates live mode only after a valid Razorpay `payment.failed` delivery. It extracts the payment amount from paise into rupees and records the error code, error source, error reason, payment method, case ID, and receipt time. Customer contact and email are not returned to the dashboard.

The default inactivity timeout is 300 seconds. Configure it with `RAZORPAY_LIVE_WEBHOOK_INACTIVITY_SECONDS`. When the timeout expires, `/api/v1/telemetry/status` reports `mode: "demo"`, and the React dashboard restores the five synthetic scenarios and their editable merchant controls. The generic `/api/v1/intake/webhook` route remains available for normalization tests but does not activate live mode because it is not the authenticated Razorpay receiver.
...
The default `data/*.jsonl`, SQLite files, and `app/artifacts/*.joblib` outputs are ignored by Git. They remain on the local machine and are regenerated by the setup commands when another evaluator clones the repository.
...
The model uses EconML's `TLearner` with independent Scikit-Learn `GradientBoostingClassifier(max_depth=4)` outcome models for the supported action arms. The probability adapter exposes `P(pay)` to EconML so `effect()` returns a continuous treatment effect rather than hard class labels. Training features include amount, case age, merchant budget, Razorpay failure telemetry, failure class, and customer segment. The implementation trains only on records that are both:
...
flowchart TD
    ...
    F --> G4[message arm]
    G0 --> H[Fit one GradientBoostingClassifier per available arm]
    G1 --> H
    G2 --> H
    G3 --> H
    G4 --> H
...
- Telemetry source: <http://127.0.0.1:8000/api/v1/telemetry/status>
